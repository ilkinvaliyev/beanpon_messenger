...
func main() {
	...
	//
	// Issue 63 (konfiq tərəfi): `INTERNAL_SECRET` TƏYİN EDİLMƏYİBSƏ hər iki
	// tərəf boş olur və `subtle.ConstantTimeCompare([]byte(""), []byte(""))`
	// 1 QAYTARIR — yəni başlığı ÜMUMİYYƏTLƏ göndərməyən İSTƏNİLƏN istək
	// /internal/* marşrutlarından keçirdi. Bu marşrutlar inzibati
	// əməliyyatlardır (otağı bitir, chat təmizlə, kick/mute, shadow-ban,
	// çağrı siqnalı, XMPP auth) — konfiq unudulduqda internetə açıq qalırdı.
	//
	// FAIL-CLOSED: sirr boşdursa marşrutlar TAMAMİLƏ bağlanır (hər istək 401)
	// və başlanğıcda uca səslə loglanır ki, səbəb dərhal görünsün.
	internalSecretSet := strings.TrimSpace(cfg.InternalSecret) != ""
	if !internalSecretSet {
		log.Println("🔴 TƏHLÜKƏSİZLİK: INTERNAL_SECRET TƏYİN EDİLMƏYİB (və ya boşdur) — " +
			"/internal/* marşrutlarının HAMISI bağlandı, hər istək 401 alacaq. " +
			"Laravel→Go inteqrasiyası (live otaq idarəsi, çağrı siqnalı, XMPP auth) " +
			"İŞLƏMƏYƏCƏK. Dəyəri mühit dəyişənlərinə əlavə edin.")
	}
	internal := router.Group("/internal")
	internal.Use(func(c *gin.Context) {
		// Boş sirr = qapı bağlı. Müqayisəyə heç düşmürük ki, "boş == boş"
		// halı təsadüfən icazə verməsin.
		if !internalSecretSet {
