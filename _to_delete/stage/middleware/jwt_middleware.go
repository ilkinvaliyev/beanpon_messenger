...
// JWTParseOptions — Issue 17: eyni sərt yoxlamaların `middleware` paketindən
// KƏNARDA da işlədilməsi üçün ixrac örtüyü.
//
// XMPP yolu (xmpp/auth_handler.go `verifyJWT`, xmpp/token_handler.go
// `bearerUserID`) `jwt.Parse`-i seçimsiz çağırırdı — yəni `exp` tələbi,
// `iss`/`aud` yoxlamaları və imza metodu ağ siyahısı ORADA TƏTBİQ OLUNMURDU.
// Müddəti olmayan (əbədi) bir token HTTP API-də rədd edilib XMPP-də qəbul
// olunurdu. Hər iki yol indi BU funksiyadan keçir.
func JWTParseOptions() []jwt.ParserOption { return jwtParseOptions() }
