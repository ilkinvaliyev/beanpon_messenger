...
func bearerUserID(c *gin.Context, secret string) (uint, bool) {
	...
	// Issue 17: HTTP middleware-i ilə EYNİ sərt seçimlər (exp tələbi,
	// iss/aud, imza metodu ağ siyahısı). Bu endpoint XMPP token-i BASIR —
	// müddətsiz bir token burada qəbul olunsaydı, ondan hər dəfə təzə XMPP
	// token-i almaq mümkün olardı (ləğvetmə tamamilə mənasızlaşırdı).
	token, err := jwt.Parse(tokenString, func(t *jwt.Token) (interface{}, error) {
		if _, ok := t.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, jwt.ErrSignatureInvalid
		}
		return []byte(secret), nil
	}, middleware.JWTParseOptions()...)
