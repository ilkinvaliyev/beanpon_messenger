...
func (h *GroupHandler) GetMyGroups(c *gin.Context) {
	...
	// Issue 57: bu endpoint BÜTÜN qrupları LIMIT-siz qaytarırdı və hər sətir
	// üçün 3 korrelyasiyalı alt-sorğu (üzv sayı, oxunmamış sayı, son mesaj)
	// işləyirdi. 500 qrupu olan istifadəçi tək istəklə minlərlə alt-sorğu
	// yaradırdı. İndi səhifələnir.
	//
	// GERİYƏ UYĞUNLUQ: parametr GÖNDƏRMƏYƏN köhnə istemçi işləməyə davam edir
	// (default 50 ilə məhdudlaşır) və cavabdakı `has_more` ilə sonradan
	// səhifələməyə keçə bilər. Mövcud sahələrin (`groups`, `count`) forması
	// dəyişmir.
	...
	// Yuxarı hədd — nəhəng offset Postgres-i mənasız yerə işlədir.
	if offset > 100000 {
		offset = 100000
	}
	...
	database.DB.Raw(`
		...
		WHERE cp.user_id = ?
		  ...
		  `+archivedWhere+`
		-- LIMIT/OFFSET ilə sıralama STABİL olmalıdır: yuxarıdakı açarların heç
		-- biri unikal deyil. Tiebreaker olmadan eyni açara sahib qruplar iki
		-- səhifədə təkrarlana və ya heç birinə düşməyə bilər.
		ORDER BY
			...
			c.id DESC
		LIMIT ? OFFSET ?
	`, userID, userID, userID, userID, userID, limit+1, offset).Scan(&groups)

	// Issue 57: `limit+1` çəkilir — əlavə sətir gəldisə "daha var" deməkdir.
	// Ayrıca COUNT sorğusuna ehtiyac qalmır.
