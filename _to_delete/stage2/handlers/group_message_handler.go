...
func (h *GroupMessageHandler) SendGroupMessage(c *gin.Context) {
	...
	// Issue 15: ÜÇÜNCÜ message_reads yazma nöqtəsi — digər ikisi kimi
	// idempotent olmalıdır. Eyni (message_id, user_id) cütü paralel yollardan
	// (retry, avto-oxundu) gələ bilir və unikal indeks pozuntusu göndərməni
	// uğursuz göstərirdi. Xəta artıq udulmur — loglanır, amma istək POZULMUR
	// (mesaj onsuz da yazılıb; göndərənin öz "oxundu" qeydi kritik deyil).
	if err := database.DB.
		Clauses(clause.OnConflict{DoNothing: true}).
		Create(&messageRead).Error; err != nil {
		log.Printf("göndərən avto-oxundu qeydi yazıla bilmədi (msg=%s, user=%d): %v",
			messageID, senderID, err)
	}
...
func (h *GroupMessageHandler) GetGroupMessages(c *gin.Context) {
	...
	// ── Issue 20: KEYSET (cursor) sayfalama — DM yolu ilə EYNİ sxem ────────
	//
	// Əvvəl kursorlar TƏK açarla (`m.created_at >= ?` / `m.created_at < ?`)
	// müqayisə olunurdu. `created_at` UNİKAL DEYİL — toplu/sürətli göndərmə
	// eyni timestamp-ı paylaşır. Nəticə:
	//   • `before` ilə yuxarı sürüşdükdə sərhəddəki eyni-ms sətirlər ya
	//     HƏMİŞƏLİK düşürdü, ya da təkrar-təkrar qayıdırdı;
	//   • `LIMIT/OFFSET` isə canlı mesaj gəldikcə siyahının başını böyüdüb
	//     səhifə sərhədlərini sürüşdürürdü → aradan mesaj ATLANIRDI.
	// İndi müqayisə TAM sıralamadır — `(created_at, id)` cütü —
	// `ORDER BY m.created_at DESC, m.id DESC` ilə uyğun.
	//
	// GERİYƏ UYĞUNLUQ: `page`/`around`/`before` parametrlərinin ADLARI və
	// mənaları dəyişmir; kursor verilmədikdə davranış əvvəlki kimi
	// page/offset-dir. Köhnə istemçilər təsirlənmir.

	// `messages.id` UUID sütunudur → kursor sentinel-ləri də ETİBARLI UUID
	// olmalıdır. Sorğuda `?::uuid` var və Postgres `''::uuid`-i sabit
	// qatlamada DƏRHAL rədd edir (`?::timestamptz IS NULL` qısa-qapanması
	// buna ZƏMANƏT vermir).
	...
	aroundCursorID := minUUID
	if aroundID != "" {
		var ts time.Time
		// Anchor yalnız BU söhbətdən ola bilər — əvvəl id qlobal axtarılırdı
		// və yad qrupun mesaj vaxtını yoxlamağa imkan verirdi.
		if err := database.DB.Raw(
			`SELECT created_at FROM messages WHERE id = ?::uuid AND conversation_id = ?`,
			aroundID, conversationID,
		).Scan(&ts).Error; err == nil && !ts.IsZero() {
			aroundCreatedAt = &ts
			aroundCursorID = aroundID
			offset = 0
			if limit < 300 {
				limit = 300
			}
	...
	// `before_ms` — anchor sətri server tərəfdə SİLİNİBSƏ işlənən ehtiyat
	// kursor (istemçidəki `created_at`, epoch ms) — DM yolu ilə eyni.
	...
	if beforeCreatedAt == nil && beforeMs > 0 {
		// MİLLİSANİYƏ YUVARLAQLAŞMASI: `created_at` mikrosaniyəlidir, `before_ms`
		// isə ms-ə kəsilib. Pəncərəni bir ms İRƏLİ sürüb sentinel kimi ƏN BÖYÜK
		// uuid-i veririk ki, həmin ms dilimi TAM daxil olsun. Təkrar sətir
		// zərərsizdir (istemçi id ilə dedup edir); İTKİ geri qaytarıla bilməz.
		...
		// Boş, "daha yoxdur" səhifəsi təhlükəsizdir.
		c.JSON(http.StatusOK, gin.H{
			"data":                    []gin.H{},
			"page":                    page,
			"limit":                   limit,
	...
	database.DB.Raw(`
		...
		WHERE m.conversation_id = ?
		  ...
		  -- Issue 20: kursorlar TAM SIRALAMA ilə müqayisə olunur —
		  -- (created_at, id) cütü. created_at unikal olmadığı üçün tək
		  -- açarlı müqayisə sərhəd sətirlərini ATLAYIR/TƏKRARLAYIRDI.
	...
	`, userID, conversationID, joinedAtFilter, me.ClearedAt,
		aroundCreatedAt, aroundCreatedAt, aroundCursorID,
	...
	// Issue 20: `beforeCreatedAt == nil` şərti — `before_ms` ilə (id-siz)
	// gələn keyset səhifəsində də ayrac hesablanmasın. O yalnız çatın İLK
	// açılışına aiddir (DM yolundakı `keysetMode` yoxlaması ilə eyni).
	if !peek && page == 1 && aroundID == "" && beforeID == "" && beforeCreatedAt == nil {
		...
		err := database.DB.Raw(`
			...
			WHERE m.conversation_id = ?
			  ...
			  AND m.created_at > COALESCE(?, '1970-01-01'::timestamptz)
			  AND mr.id IS NULL
			-- Issue 20: created_at unikal deyil. Bu id around kursoru kimi geri
			-- qayıdır və orada müqayisə TAM sıralamadır — (created_at, id).
			-- Tiebreak olmadan eyni-ms qrupundan TƏSADÜFİ sətir seçilir və
			-- ondan KİÇİK id-li oxunmamışlar ayracın YUXARISINDA qalır.
	...
	hasOlder := false
	// Issue 20: növbəti kursor — səhifədəki ƏN KÖHNƏ mesaj (siyahı
	// `created_at DESC, id DESC` sıralıdır → sonuncu element ən köhnə).
	var nextBeforeID *string
	if len(messages) > 0 {
		oldest := messages[len(messages)-1]
		nextBeforeID = &oldest.ID
		var cnt int64
		// "Daha köhnə var?" yoxlaması da EYNİ tam sıralama ilə — `created_at <`
		// tək başına eyni-ms sətirləri gizlədib "tarixçə bitdi" deyirdi.
		// Bloklama süzgəci də siyahı sorğusundakı ilə EYNİ olmalıdır — yoxsa
		// qalan bütün köhnə mesajlar bloklanmış istifadəçidəndirsə `has_older`
		// true qayıdır, növbəti səhifə isə BOŞ gəlir (sonsuz spinner).
		database.DB.Model(&models.Message{}).
			Where(`conversation_id = ? AND deleted_at IS NULL AND (created_at, id) < (?, ?::uuid) AND created_at >= ? AND created_at > COALESCE(?, '1970-01-01'::timestamptz)
				AND NOT EXISTS (
				    SELECT 1 FROM user_blocks ub
				    WHERE (ub.blocker_id = ? AND ub.blocked_id = messages.sender_id)
				       OR (ub.blocker_id = messages.sender_id AND ub.blocked_id = ?)
				)`,
				conversationID, oldest.CreatedAt, oldest.ID, joinedAtFilter, me.ClearedAt, userID, userID).
			Limit(1).Count(&cnt)
		hasOlder = cnt > 0
	}
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"has_older": hasOlder,
		// Issue 20 (additiv): növbəti səhifə üçün `before` kursoru. Köhnə
		// istemçilər bu sahəni görməzdən gəlir və `page` ilə davam edir.
...
func (h *GroupMessageHandler) GetGroupStarred(c *gin.Context) {
	...
	// Issue 57: LIMIT yox idi — ulduzlanmış mesaj sayı sərbəst artdığından
	// (istifadəçi başına minlərlə ola bilər) hər çağırış bütün sətirləri
	// çəkib deşifrə edirdi. GERİYƏ UYĞUNLUQ: parametrsiz köhnə istemçi işləyir
	// (default 50), `has_more` ilə sonradan səhifələyə bilər.
	limit, _ := strconv.Atoi(c.DefaultQuery("limit", "50"))
	...
	database.DB.Raw(`
		...
		ORDER BY m.created_at DESC, m.id DESC
		LIMIT ? OFFSET ?
	`, conversationID, userID, limit+1, offset).Scan(&rows)
