...
func (h *RaveHandler) HandleWebSocket(c *gin.Context) {
	...
	// Struct literal DEYİL, konstruktor: client-in daxili kapanış siqnalı
	// (`done`) ixrac olunmur və yalnız burada düzgün qurulur.
	client := websocket.NewRaveClient(
		h.hub, conn, userID, raveID, rave.UserID == userID, user.Name, user.Avatar,
	)
