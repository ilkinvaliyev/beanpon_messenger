...
type IngressSink interface {
	...
	//
	// clientMessageID is the stanza id supplied by the sending client. It is
	// the idempotency key: the REST path takes the same value as
	// `client_message_id` and inserts with ON CONFLICT (id) DO NOTHING, so a
	// resent stanza (stream resumption, offline queue replay) does not create a
	// second row. It is untrusted input — the Hub validates it as a UUID and
	// falls back to a server-generated id when it is empty or malformed.
	IngestGroup(senderID, conversationID uint, text, kind, clientMessageID string)
...
func (b *Bridge) OnMessage(s Message) {
	...
	case "groupchat":
		if gm, ok := FromGroupStanza(s); ok {
			b.sink.IngestGroup(gm.SenderID, gm.ConversationID, gm.Text, gm.Kind, gm.MessageID)
		}
	}
}
