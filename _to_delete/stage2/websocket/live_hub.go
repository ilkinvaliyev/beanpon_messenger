...
type LiveRoomClient struct {
	...
	//
	// `Send` kanalı ARTIQ HEÇ VAXT bağlanmır; bağlanma siqnalı `done`-dur.
	// Əvvəl `close(client.Send)` həm `Unregister` budağında (yazma kilidi
	// altında), həm də ONLARLA "yavaş istehlakçı" `default:` budağında
	// (kilidsiz və ya yalnız RLock altında) çağırılırdı. İki nəticə:
	//   • İKİQAT close → panic → bütün proses çökür;
	//   • bir goroutine bağlayarkən digəri həmin kanala yazır →
	//     "send on closed channel" → yenə tam proses çöküşü.
	// Fan-out kilid ALTINDAN çıxarıldıqda bu pəncərə daha da genişləyirdi,
	// ona görə hub.go-dakı eyni qoruma buraya da gətirildi: `Send`-ə paralel
	// non-blocking yazımlar həmişə təhlükəsizdir, writePump isə `done` ilə
	// dayanır.
	closeOnce sync.Once
	done      chan struct{}
...
// `done`-u bağlayır. İstənilən qədər, istənilən goroutine-dən çağırıla bilər.
func (c *LiveRoomClient) closeSend() {
	if c == nil {
		return
	}
	c.closeOnce.Do(func() {
...
// liveRoomSnapshot — otaq client-lərinin KİLİD ALTINDA götürülmüş nüsxəsi.
//
// Issue 23 (hub.go-dakı `statusTargetsLocked` ilə eyni səbəb): fan-out
// `h.mu.RLock()` altında edilirdi — yəni bir yavaş/tıxanmış client üçün
// `select` gözləməsi və N addımlıq döngü boyunca BÜTÜN hub (register,
// unregister, hər `handleEvent`) dondurulurdu. İndi kilid altında yalnız
// göstəricilər kopyalanır (sabit və qısa iş), kanal yazıları kilid
// BURAXILDIQDAN sonra baş verir.
func (h *LiveHub) liveRoomSnapshot(roomID uint) []*LiveRoomClient {
	h.mu.RLock()
	...
	out := make([]*LiveRoomClient, 0, len(room))
	for _, c := range room {
		out = append(out, c)
	}
	h.mu.RUnlock()
...
func (h *LiveHub) handleEvent(event *LiveMessageEvent) {
	...
	case "kick_from_live":
		...
		// Issue 23: alıcılar kilid altında SNAPSHOT edilir, kanal yazıları
		// kilid buraxıldıqdan SONRA (bax liveRoomSnapshot).
		for _, c := range h.liveRoomSnapshot(event.RoomID) {
			p := othersPayload
...
func (h *LiveHub) ClearChat(c *gin.Context) {
	...
	// Issue 23: snapshot kilid altında, göndərmə kiliddən KƏNARDA.
	for _, client := range h.liveRoomSnapshot(uint(roomID)) {
		select {
